...
describe("PositionManager next short data calculations", function () {
  ...
  async function debugState(label = "") {
    ...
    const aum = await klpManager.getAum(true)
  ...
  it("PositionManager and KlpManager init with shortsTracker", async () => {
    const [
      positionManagerShortTracker,
      klpManagerShortTracker,
      avgeragePrice,
      size
    ] = await Promise.all([
      positionManager.shortsTracker(),
      klpManager.shortsTracker(),
    ...
    expect(klpManagerShortTracker, 'klpManager shortsTracker').eq(shortsTracker.address)
  ...
  it("updates global short average prices on position decreases", async () => {
    await shortsTracker.setIsGlobalShortDataReady(true)
    await klpManager.setShortsTrackerAveragePriceWeight(10000)
    expect(await klpManager.shortsTrackerAveragePriceWeight()).to.be.equal(10000)
    ...
    aumAfter = await klpManager.getAum(true)
