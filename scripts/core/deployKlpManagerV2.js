...
async function getArbValues() {
  ...
  return {vault, usdg, klp, shortsTracker}
}
...
async function main() {
  const {vault, usdg, klp, shortsTracker} = await getValues()

  const cooldownDuration = 0
  const klpManager = await deployContract("KlpManager", [vault.address, usdg.address, klp.address, shortsTracker.address, cooldownDuration])
